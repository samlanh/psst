...
class Application_Model_DbTable_DbGradingScore extends Zend_Db_Table_Abstract
{
	...
	function getAllGradingScore($search=null){
		...
		if(!empty($search['subjectId'])){
			$where.=" AND `grd`.`subjectId` =".$search['subjectId'];
		}
...
// 			$this->submittoGradingTotal($data);
   ...
   public function UpdateScoreGradingByClass($_data){
   	$db = $this->getAdapter();
   	$db->beginTransaction();
   	try{
   		...
   		$where='gradingId='.$recordId;
   		$this->_name='rms_grading_detail_tmp';
   		$this->delete($where);
   		
   		$where='gradingTmpId='.$recordId;
   ...
   function getGradingScoreById($gradingId,$fullControlID){
   	$db=$this->getAdapter();
