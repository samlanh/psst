					...
					$string.='<div class="form-group">';
						if(!empty($rowCri['subjectId'])){
							...
							for ($x = 1; $x <= $rowCri['timeInput']; $x++) {
								...
								$string.='<div class="'.$classCol.'">';
								$string.='<input '.$readonly.' value="'.$attScore.'" data-dojo-props="constraints:{min:0,max:'.$maxSubjectScore.'},'.$invalidesms.'" required="1" class="fullside" dojoType="dijit.form.NumberTextBox" type="text" onKeyup="calculateAverage('.$keyIndex.')"  name="score_'.$keyIndex.'_'.$x.$criterialId.'" id="score_'.$keyIndex.'_'.$x.$criterialId.'"  />';
								$string.='</div>';
							}
						}
					$string.='</div>';
