    ...
    public function getStudentPaymentByid($id){
    	...
    	$sql = "SELECT 
    			   ...
    			   sp.number,
					...
					(SELECT $grade FROM rms_itemsdetail WHERE rms_itemsdetail.id=(SELECT gds.grade FROM rms_group_detail_student AS gds WHERE gds.mainType=1 AND gds.stu_id = s.stu_id AND gds.is_current=1 AND gds.is_maingrade=1 LIMIT 1) AND rms_itemsdetail.items_type=1 LIMIT 1)AS gradeTitle,
					(SELECT group_code FROM `rms_group` WHERE rms_group.id=(SELECT ds.group_id FROM rms_group_detail_student AS ds 
					WHERE ds.mainType=1 AND ds.stu_id=s.stu_id AND ds.is_maingrade=1 AND ds.grade=sp.grade AND ds.degree=sp.degree ORDER BY ds.gd_id DESC LIMIT 1) LIMIT 1) AS group_name
    			FROM
