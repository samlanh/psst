...
class Foundation_Model_DbTable_DbFamily extends Zend_Db_Table_Abstract
{
	...
	function checkMotherKhHasInFamilyId($motherNameKh){
		$db = $this->getAdapter();
		$sql = "
			SELECT 
				s.*
			FROM `rms_family` AS s 
			WHERE 1
			AND s.motherNameKh = '$motherNameKh' 
			AND (s.`fatherNameKh` IS NULL OR s.`fatherNameKh` ='')
			LIMIT 1
		";
		$rs = $db->fetchRow($sql);
		return $rs;
	...
	function getDistinctParentMotherKhNameStudent(){
		$db = $this->getAdapter();
		$sql = "
			SELECT 
				DISTINCT s.`mother_khname`
				,s.*
			FROM `rms_student` AS s 
			WHERE s.`mother_khname` IS NOT NULL AND s.`mother_khname` !='' AND (s.`father_khname` IS NULL OR s.`father_khname` ='')
			AND COALESCE(s.`familyId`,0) = 0
			GROUP BY s.`mother_khname`
		";
		$rs = $db->fetchAll($sql);
		return $rs;
	}
	...
	function updateFamilyIdInStudentOldData(){
		...
		$rsMother = $this->getDistinctParentMotherKhNameStudent();
		if(!empty($rsMother)){
			foreach($rsMother as $row){
				
				$check = $this->checkMotherKhHasInFamilyId($row["mother_khname"]);
				if(!empty($check)){
					$_arr= array(
						'familyId'	=>$check['id'],
					);
					$this->_name = "rms_student";
					$where = " mother_khname = '".$row["mother_khname"]."'";
					$this->update($_arr, $where);	
				}else{
