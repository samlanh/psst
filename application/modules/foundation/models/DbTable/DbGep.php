...
class Foundation_Model_DbTable_DbGep extends Zend_Db_Table_Abstract
{
	...
	public function getAllStudent($search){
		...
		(select CONCAT(from_academic,'-',to_academic,'(',generation,')',' ',(select name_en from rms_view where type=7 and key_code = time)) from rms_tuitionfee where rms_tuitionfee.id=academic_year) as academic,
	...
	public function addStudent($_data){
			...
			$stu_code = $code->getNewAccountNumber($_data['degree'], 2);
			try{	
				$_db= $this->getAdapter();
				$_arr= array(
...
// 					'to_time'=>$_data['to_time'],
	...
	function getAllGrade($grade_id){
		$db = $this->getAdapter();
		$sql = "SELECT major_id As id,CONCAT(major_enname,' - ',major_khname) As name FROM rms_major WHERE dept_id=".$grade_id;
