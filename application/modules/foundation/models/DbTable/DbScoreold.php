...
// 			(SELECT CONCAT(name_en ,'-',name_kh ) FROM rms_view WHERE `type`=7 AND rms_view.key_code=`g`.`time`) AS TIME,
// 			s.status
// 			FROM rms_score AS s,rms_group AS g WHERE s.group_id=g.id AND g.degree IN(1,2) AND s.status=1";
		...
		(SELECT CONCAT(name_en ,'-',name_kh ) FROM rms_view WHERE `type`=7 AND rms_view.key_code=`g`.`time`) AS TIME,
		s.status
		FROM rms_score AS s,rms_group AS g WHERE s.group_id=g.id  AND s.status=1 AND `g`.`degree` IN(1,2)";
		$where ='';
