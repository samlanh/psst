<?php
class Gep_studentchangegroupController extends Zend_Controller_Action {
	...
	public function indexAction(){
		...
		$db_student= new Gep_Model_DbTable_DbStudentChangeGroup();
	...
	function addAction(){
		if($this->getRequest()->isPost()){
			try{
				...
				$_add = new Gep_Model_DbTable_DbStudentChangeGroup();
 				$_add->addStudentChangeGroup($_data);
 				if(!empty($_data['save_close'])){
 					Application_Form_FrmMessage::Sucessfull("INSERT_SUCCESS","/gep/studentchangegroup");
 				}
...
// 		$this->view->province = $row =$_db->getProvince();
	...
	public function editAction(){
		...
		$db= new Gep_Model_DbTable_DbStudentChangeGroup();
		...
		if($this->getRequest()->isPost())
		{
			try{
				...
				Application_Form_FrmMessage::Sucessfull("EDIT_SUCCESS","/gep/studentchangegroup/index");
			}catch(Exception $e){
