<?php
class Global_CommuneController extends Zend_Controller_Action {
	protected $tr;
	...
	public function indexAction(){
		...
		$frm = new Global_Form_FrmCommune();
	...
	public function addAction(){
		if($this->getRequest()->isPost()){
			...
			try{
				$db_district = new Global_Model_DbTable_DbCommune();				
	...
	public function editAction(){
		...
		if (empty($id)){
			Application_Form_FrmMessage::Sucessfull($this->tr->translate("No Record"),self::REDIRECT_URL.'/commune/');
			exit();
		}
		$db = new Global_Model_DbTable_DbCommune();
		if($this->getRequest()->isPost()){
