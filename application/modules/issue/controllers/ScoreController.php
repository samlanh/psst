	...
	function getIssemsterAction()
	{
		if ($this->getRequest()->isPost()) {
			...
			if (!empty($row)) {
				if ($row["exam_type"] == 2) {
					$arr = array(
						"semester" => true, "annual" => false
					);
					print_r(Zend_Json::encode($arr));
					exit();
				} else if ($row["exam_type"] == 3) {
					$arr = array(
						"semester" => false, "annual" => true
					);
					print_r(Zend_Json::encode($arr));
					exit();
				} else {
					$arr = array(
						"semester" => false, "annual" => false
					);
					print_r(Zend_Json::encode($arr));
					exit();
				}
	...
	public	function publicScoreAction()
	{
		if ($this->getRequest()->isPost()) {
			...
			try {
				...
				Application_Form_FrmMessage::Sucessfull("INSERT_SUCCESS", "/allreport/score/rpt-score");
				exit();
			} catch (Exception $e) {
