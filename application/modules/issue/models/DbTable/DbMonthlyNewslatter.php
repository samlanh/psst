...
class Issue_Model_DbTable_DbMonthlyNewslatter extends Zend_Db_Table_Abstract
{
    ...
    public function addMonthlyNewslatter($_data){
    	$db = $this->getAdapter();
    	$db->beginTransaction();
    	try{
    			$_arr = array(
    					'branch_id'=>$_data['branch_id'],
    					'newslatter_setting'=>$_data['newsLatterSetting'],
...
//     					'title'=>$_data['title'],
    			...
    			$this->_name='rms_monthlynewslatter';
    			$id=$this->insert($_arr);
    			...
    			if(!empty($_data['identity'])){
    				...
    				if(!empty($ids))foreach ($ids as $i){
    					$arr=array(
    						...
    						'performane_id'=> $_data["performance_".$i],
    						'note'=>$_data['note_'.$i],
    						'user_id'=>$this->getUserId(),
    					);
    					$this->_name='rms_monthlynewslatter_detail';
    					$this->insert($arr);
    				}
    			}
    		$db->commit();
    ...
    public function editMonthlyNewslatter($_data){
    	$db = $this->getAdapter();
    	$db->beginTransaction();
    	try{
...
//     					'title'=>$_data['title'],
    			...
    			$this->_name='rms_monthlynewslatter_detail';
    			$wheredelete = "monthly_news_id =".$id;
    ...
    function getAllScore($search=null){
    	...
    	$sql.=" FROM `rms_monthlynewslatter` AS seng,
						rms_group AS g,
						rms_student AS s
    ...
    function getMonthlyNewslatterById($score_id){
    	$db=$this->getAdapter();
    	$sql="SELECT s.*,(SELECT g.is_pass FROM `rms_group` AS g WHERE g.id = s.group_id LIMIT 1) AS is_pass FROM `rms_monthlynewslatter` AS s
			WHERE s.id= $score_id";
    ...
    function getMonthlyNewslatterDetailById($score_id){
    	$db=$this->getAdapter();
    	...
    	$sql="SELECT s.*,seng.$colunmname AS `title_sub` FROM `rms_monthlynewslatter_detail` AS s,
		`rms_subjectarea` AS seng
    	WHERE seng.id = s.subject_area_id AND s.monthly_news_id= $score_id"; 
    	return $db->fetchAll($sql);
    }
    
	function getNewsLatterSettingByBranch($branch_id){
		$db = $this->getAdapter();
		$sql="SELECT sc.id,sc.title AS `name` FROM `rms_newslattersetting` AS sc
			WHERE sc.status=1 AND sc.branch_id = $branch_id";
		return $db->fetchAll($sql);
	}
	function getNewslatterSettingDetail($id){
		$dbgb = new Application_Model_DbTable_DbGlobal();
		$currentLang = $dbgb->currentlang();
		$colunmname='title_en';
		if ($currentLang==1){
		...
		FROM
		`rms_newslattersetting_detail` AS s,
		`rms_subjectarea` AS seng
	...
	function checkuDuplicate($data){
		...
		$sql="
		SELECT
		* FROM rms_monthlynewslatter AS i
