<?php
class Issuesetting_Model_DbTable_DbScoreSetting extends Zend_Db_Table_Abstract
{
	...
	public function addScoreSetting($_data){
		$db = $this->getAdapter();
		$db->beginTransaction();
		try{
			$_arr = array(
   ...
   function getScoreSettingById($id){
   		$db = $this->getAdapter();
   		$sql="SELECT s.* FROM `rms_scoreengsetting` AS s WHERE s.id=$id AND s.schoolOption=2 ";
