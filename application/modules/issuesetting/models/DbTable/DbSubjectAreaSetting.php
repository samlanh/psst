<?php
class Issuesetting_Model_DbTable_DbSubjectAreaSetting extends Zend_Db_Table_Abstract
{
    protected $_name = 'rms_subjectarea_setting';
    ...
    function getAllScoreSetting($search = '',$items_type=null){
    	...
    	$sql.=" FROM `rms_subjectarea_setting` AS s
				WHERE 1 ";
	...
	public function addSubjectareaSetting($_data){
		$db = $this->getAdapter();
		$db->beginTransaction();
		try{
			$_arr = array(
			...
			$this->_name='rms_subjectarea_setting';
			$id = $this->insert($_arr);
			...
			if(!empty($_data['identity'])){
				...
				if(!empty($ids))foreach ($ids as $i){
					$arr=array(
							...
							'max_value'=>$_data['max_value_'.$i],
							'note'=>$_data['note_'.$i],
					);
					$this->_name='rms_subjectarea_setting_detail';
					$this->insert($arr);
				}
			}
		  $db->commit();
   ...
   function getSubjectareaSettingById($id){
   		$db = $this->getAdapter();
   		$sql="SELECT s.* FROM `rms_subjectarea_setting` AS s WHERE s.id=$id";
   ...
   function getSubjectareaSettingDetail($id){
   	$db = $this->getAdapter();
   	$sql="SELECT s.*,(SELECT es.title FROM `rms_subjectarea` AS es WHERE es.id = s.subject_area_id LIMIT 1) AS subjectarea_title FROM `rms_subjectarea_setting_detail` AS s WHERE s.setting_id=$id";
