...
class Kindergarten_Model_DbTable_DbGroupStudentChangeGroup extends Zend_Db_Table_Abstract
{
	...
	public function getfromGroup(){
		$db = $this->getAdapter();
		$sql = "SELECT group_code,id FROM `rms_group` where status = 1 and degree IN (1) ";
// 		$orderby = " ORDER BY stu_code ";
	...
	public function selectAllStudentChangeGroup($search){
		...
		$sql = "SELECT rms_group_student_change_group.id,(select group_code from rms_group where rms_group.id=rms_group_student_change_group.from_group) as group_code,
				...
				FROM `rms_group_student_change_group`,rms_group where rms_group_student_change_group.from_group=rms_group.id and rms_group.degree IN (1)";
