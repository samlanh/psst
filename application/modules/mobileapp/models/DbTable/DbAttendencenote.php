<?php
class Mobileapp_Model_DbTable_DbAttendencenote extends Zend_Db_Table_Abstract
{
	protected $_name = 'mobile_attendencenote';
...
public function getUserId(){
	...
	function getAllAttendencenote($search){
		...
		$sql="SELECT mba.id,
		(SELECT ad.title FROM `mobile_attendencenote_detail` AS ad WHERE ad.attendance_id = mba.`id` AND ad.lang=$lang LIMIT 1) AS title,
		...
		if(!empty($search['adv_search'])){
			...
			$s_where[]= " (SELECT ad.title FROM `mobile_attendencenote_detail` AS ad WHERE ad.attendance_id = mba.`id` AND ad.lang=$lang LIMIT 1) LIKE '%{$s_search}%'";
            ...
            $lang = $dbglobal->getLaguage();
        	 if (!empty($data['id'])){
        	 	...
        	 	if(!empty($lang)){
        	 		...
        	 		$this->_name="mobile_attendencenote_detail";
        	 		$where1=" attendance_id=".$data['id'];
        	 		if (!empty($iddetail)){
        	 		...
        	 		foreach($lang as $row){
        	 			...
        	 			if (!empty($data['iddetail'.$title])){
        	 				$arr_article = array(
        	 						'attendance_id'=>$article_id,
        	 						'title'=>$data['title'.$title],
        	 						'description'=>$data['description'.$title],
        	 						'lang'=>$row['id'],
        	 				);
        	 				$this->_name="mobile_attendencenote_detail";
        	 				$wheredetail=" attendance_id=".$data['id']." AND id=".$data['iddetail'.$title];
 ...
 function deleteData($id){
 	...
 	try{
 		...
 		$this->_name = "mobile_attendencenote_detail";
 		$where=$this->getAdapter()->quoteInto("attendance_id=?", $id);
 		$this->delete($where);
 ...
 function getArticleTitleByLang($article_id,$lang){
 	$db = $this->getAdapter();
 	$sql="SELECT acd.id,acd.`title`,acd.`lang`,acd.description FROM `mobile_attendencenote_detail` AS acd WHERE acd.`attendance_id`=$article_id AND acd.`lang`=$lang";
