...
class Mobileapp_Model_DbTable_DbGradingSystem extends Zend_Db_Table_Abstract
{
	...
	function getAllGradingSystem($search){
		...
		$sql="SELECT mba.id,
		(SELECT ad.title FROM `mobile_grading_system_detail` AS ad WHERE ad.grading_id = mba.`id` AND ad.lang=$lang LIMIT 1) AS title,
		mba.date,
		mba.status as status FROM $this->_name AS mba WHERE 1";
		...
		if(!empty($search['adv_search'])){
			...
			$s_where[]= " (SELECT ad.title FROM `mobile_grading_system_detail` AS ad WHERE ad.grading_id = mba.`id` AND ad.lang=$lang LIMIT 1) LIKE '%{$s_search}%'";
	...
	function add($data){
        ...
        try{
            ...
            $lang = $dbglobal->getLaguage();
        	 if (!empty($data['id'])){
        	 	...
        	 	$where=" id=".$data['id'];
        	 	$this->_name="mobile_grading_system";
        	 	...
        	 	if(!empty($lang)){
        	 		...
        	 		$this->_name="mobile_grading_system_detail";
        	 		$where1=" grading_id=".$data['id'];
        	 		if (!empty($iddetail)){
        	 		...
        	 		foreach($lang as $row){
        	 			...
        	 			if (!empty($data['iddetail'.$title])){
        	 				$arr_article = array(
        	 						'grading_id'=>$article_id,
        	 						'title'=>$data['title'.$title],
        	 						'description'=>$data['description'.$title],
        	 						'lang'=>$row['id'],
        	 				);
        	 				$this->_name="mobile_grading_system_detail";
        	 				$wheredetail=" grading_id=".$data['id']." AND id=".$data['iddetail'.$title];
 	...
 	function deleteData($id){
	 	...
	 	try{
	 		...
	 		$this->_name = "mobile_grading_system_detail";
	 		$where=$this->getAdapter()->quoteInto("grading_id=?", $id);
	 		$this->delete($where);
	 	}catch(exception $e){
	 		Application_Form_FrmMessage::message("Application Error");
	 		Application_Model_DbTable_DbUserLog::writeMessageError($e->getMessage());
	 ...
	 function getArticleTitleByLang($article_id,$lang){
	 	$db = $this->getAdapter();
	 	$sql="SELECT acd.id,acd.`title`,acd.`lang`,acd.description FROM `mobile_grading_system_detail` AS acd WHERE acd.`grading_id`=$article_id AND acd.`lang`=$lang";
