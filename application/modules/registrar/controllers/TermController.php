<?php
class Registrar_TermController extends Zend_Controller_Action {
...
//     		$rs_rows= $db->getAllStudentTest($search);
    	...
    	if($this->getRequest()->isPost()){
	    	try{
	    		...
	    		if(isset($data['save_close'])){
					Application_Form_FrmMessage::Sucessfull("INSERT_SUCCESS","/registrar/term");
				}else{
					Application_Form_FrmMessage::Sucessfull("INSERT_SUCCESS","/registrar/term/add");
				}
	    	}catch(Exception $e){
	...
	public function editAction(){
		...
		if($this->getRequest()->isPost()){
	    	try{
	    		$data = $this->getRequest()->getPost();
	    		$db->editTerm($data,$id);
				Application_Form_FrmMessage::Sucessfull("EDIT_SUCCESS","/registrar/term");
	    	}catch(Exception $e){
	    		Application_Form_FrmMessage::message("APPLICATION_ERROR");
	    		Application_Model_DbTable_DbUserLog::writeMessageError($e->getMessage());
	    	}
	...
	function addtermAction(){
		if($this->getRequest()->isPost()){
			$data=$this->getRequest()->getPost();
			$dbc = new Accounting_Model_DbTable_DbStartdateEnddate();
