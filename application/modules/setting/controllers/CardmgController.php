<?php
class Setting_CardmgController extends Zend_Controller_Action {
	...
	public function indexAction(){
      try{
    	$db = new Setting_Model_DbTable_DbCardmg();
    	 if($this->getRequest()->isPost()){
           ...
           $rs_rows= $db->getAllBranch($search);
			...
			$link=array(
					      'module'=>'setting','controller'=>'cardmg','action'=>'edit',
			);
	...
	function addAction()
	{
		if($this->getRequest()->isPost()){//check condition return true click submit button
			...
			try {
				$sms = "INSERT_SUCCESS";
				$_dbmodel = new Setting_Model_DbTable_DbCardmg();
