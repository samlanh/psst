...
var WebCodeCamJS = function(element) {
    ...
    navigator.mediaDevices.getUserMedia = function(c) {
        ...
        if (ua.match(/iPhone|iPad/)) {
        	return new Promise(function(y, n) {
                (navigator.mediaDevices.getUserMedia || navigator.mozGetUserMedia || navigator.webkitGetUserMedia || navigator.userAgent).call(navigator, c, y, n);
            });
        }else{
        	return new Promise(function(y, n) {
                (navigator.getUserMedia || navigator.mozGetUserMedia || navigator.webkitGetUserMedia || navigator.userAgent).call(navigator, c, y, n);
            });
        }
    ...
    var display = Q(element),
        ...
        options = {
            ...
            zoom: 0,
            beep: urlpart+"/templates/"+'plugin/webcodecamjs/audio/beep.mp3',
            decoderWorker: urlpart+"/templates/"+'plugin/webcodecamjs/js/DecoderWorker.js',
            brightness: 0,
...
// ----ここから追記----
	...
	function checkDigit(barcodeStr, barCodeType) {
        // Change the processing for each barcode type 
        if (barCodeType === "EAN-13") {
            ...
            for (var i = 0; i < barcodeStr.length - 1; i++) {
                if (i % 2 === 0) {  // Whether it is "odd" (because it starts from 0, even and odd of i are reversed) 
        ...
        // // process passes without true (check if the bar code does not qualify) 
        return true;
    }
    ...
    var numCamerarUnknow=0;
    function gotSources(device) {
    	
        if (device.kind === 'video' || device.kind === 'videoinput') {
        	numCamerarUnknow = numCamerarUnknow+1
           // var face = (!device.facing || device.facing === '') ? 'unknown' : device.facing;
        	var face = (!device.facing || device.facing === '') ? numCamerarUnknow : device.facing;
            var text = device.label || 'camera ' + (videoSelect.length + 1) + ' (facing: ' + numCamerarUnknow + ')';
